...
def test_lenet(model, data_test_loader):
...
def test_cpm(model, data_test_loader, test_batch_size):
    # set_random_seed(SEED)
    ...
    for batch_data in data_test_loader:
        test_img = batch_data['image'].cuda()
        test_hmap = batch_data['heatmap'].cuda()

        pred_hmaps_list = model(test_img)

        loss_value = 0
        for hmap in pred_hmaps_list:
            loss_value += ((hmap - test_hmap) ** 2).sum().item() / test_batch_size
        loss_value_list.append(loss_value)
